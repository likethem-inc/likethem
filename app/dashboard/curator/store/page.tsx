...
export default function StorePage() {
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setErrorMessage(t('store.errors.fileSizeLarge'))
      setShowErrorToast(true)
      return
    }
    ...
    if (!file.type.match(/image\/(jpeg|jpg|png|webp)/)) {
      setErrorMessage(t('store.errors.fileTypeInvalid'))
      setShowErrorToast(true)
      return
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : t('store.errors.failedToSave'))
      setShowErrorToast(true)
  ...
  const handleCancel = () => {
    if (hasUnsavedChanges) {
      if (confirm(t('store.unsavedChanges'))) {
    ...
    <div className="min-h-screen bg-gray-50 py-24">
      <div className="container-custom max-w-4xl">
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              >
                ...
                <span>{t('store.backToDashboard')}</span>
              </Link>
            </div>
            <div className="flex items-center space-x-4">
              {profile.slug && (
                ...
                >
                  ...
                  <span>{t('store.previewStore')}</span>
                </a>
              )}
            </div>
          </div>
          ...
          <div className="mt-6">
            <h1 className="font-serif text-3xl font-light mb-2">{t('store.title')}</h1>
          ...
          {profile.isEditorPick && (
            <div className="mt-4 flex items-center space-x-2">
              ...
              <span className="text-sm font-medium text-yellow-700">{t('store.editorsPick')}</span>
            </div>
          )}
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="font-serif text-xl font-light mb-6">{t('store.basicInfo.title')}</h2>
              
              <div className="space-y-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {t('store.basicInfo.displayName')}
                  </label>
                  <input
                    type="text"
                    value={profile.name}
                    ...
                    placeholder={t('store.basicInfo.displayNamePlaceholder')}
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    {t('store.basicInfo.charCount', { count: profile.name.length, max: 50 })}
                  </p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {t('store.basicInfo.bio')}
                  </label>
                  <textarea
                    value={profile.bio}
                    ...
                    placeholder={t('store.basicInfo.bioPlaceholder')}
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    {t('store.basicInfo.charCount', { count: profile.bio.length, max: 280 })}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-carbon"
                      placeholder={t('store.basicInfo.locationPlaceholder')}
                    />
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      {t('store.basicInfo.styleTags')}
                    </label>
                    <input
                      type="text"
                      value={profile.style}
                      ...
                      className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-carbon"
                      placeholder={t('store.basicInfo.styleTagsPlaceholder')}
                    />
                  </div>
                </div>
              </div>
            ...
            {/* Store Tags */}
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="font-serif text-xl font-light mb-6">{t('store.storeTags.title')}</h2>
              
              <div className="space-y-4">
                <p className="text-sm text-gray-600">
                  {t('store.storeTags.subtitle')}
                </p>
            ...
            {/* Social Links */}
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="font-serif text-xl font-light mb-6">{t('store.socialMedia.title')}</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-carbon"
                    placeholder={t('store.socialMedia.channelUrl')}
                  />
                </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:border-carbon"
                    placeholder={t('store.socialMedia.websitePlaceholder')}
                  />
                </div>
              </div>
            </div>
            ...
            {/* Store Settings */}
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="font-serif text-xl font-light mb-6">{t('store.settings.title')}</h2>
              
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-medium text-gray-900">{t('store.settings.storeStatus')}</h3>
                    <p className="text-sm text-gray-600">
                      {profile.isPublic ? t('store.settings.statusPublic') : t('store.settings.statusPrivate')}
                    </p>
                  </div>
                  <button
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-medium text-gray-900">{t('store.settings.editorsPick')}</h3>
                    <p className="text-sm text-gray-600">
                      {t('store.settings.editorsPickDesc')}
                    </p>
                  </div>
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-500">
                      {profile.isEditorPick ? t('store.settings.active') : t('store.settings.inactive')}
                    </span>
                  </div>
                </div>
              </div>
          ...
          >
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="font-serif text-xl font-light mb-6">{t('store.profilePicture.title')}</h2>
              ...
              <div className="space-y-4">
                ...
                <div className="text-center">
                  <p className="text-xs text-gray-500 mb-2">
                    {t('store.profilePicture.hint')}
                  </p>
                  <p className="text-xs text-gray-500">
                    {t('store.profilePicture.format')}
                  </p>
                </div>
              </div>
            </div>
            ...
            {/* Banner Upload */}
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="font-serif text-xl font-light mb-6">{t('store.banner.title')}</h2>
            ...
            {/* Badges */}
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h2 className="font-serif text-xl font-light mb-6">{t('store.badges.title')}</h2>
                ...
                >
                  {isSaving ? (
                    <>
                      ...
                      <span>{t('store.actions.saving')}</span>
                    </>
                  ) : (
                    <>
                      <Save className="w-4 h-4" />
                      <span>{t('store.actions.saveChanges')}</span>
                    </>
                  )}
                </button>
      ...
      {showSuccessToast && (
        ...
        >
          ...
          <span>{t('store.toast.success')}</span>
          <button
