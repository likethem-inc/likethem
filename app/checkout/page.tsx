...
export default function CheckoutPage() {
  ...
  // Fetch product details and curator-specific payment methods on mount
  useEffect(() => {
    const fetchProductDetailsAndPaymentMethods = async () => {
      ...
      try {
        ...
        // Step 1: Fetch product details for all cart items
        const productPromises = items
          .filter(item => item.productId) // Only fetch for items with productId
          .map(async (item) => {
            try {
              ...
              console.error(`[checkout] Failed to fetch product ${item.productId}:`, error)
              return null
            }
          })

        const productResults = await Promise.all(productPromises)
        const productsMap = new Map<string, ProductDetails>()
        
        productResults.forEach(result => {
          if (result && result.product) {
            productsMap.set(result.productId, result.product)
          }
        })
        ...
        // Check if cart has items from multiple curators
        if (curatorIds.size > 1) {
          setMultiCuratorWarning(true)
          console.warn('[checkout] Cart contains items from multiple curators:', Array.from(curatorIds))
        }

        // Step 3: Get the first curator ID (for MVP, we'll use the first curator)
        const firstCuratorId = Array.from(curatorIds)[0]
        
        if (!firstCuratorId) {
          throw new Error('No curator found for cart items')
        }

        setCuratorId(firstCuratorId)

        // Step 4: Fetch payment methods for this curator
        const response = await fetch(`/api/payment-methods?curatorId=${firstCuratorId}`)
        if (!response.ok) {
                  ...
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
                    <div className="flex items-start space-x-3">
                      ...
                      <div className="text-sm text-blue-800">
                        ...
                        <p>Your cart contains items from multiple curators. For this checkout, we&apos;ll use the payment methods of the first curator. Separate orders will be created for each curator.</p>
                      </div>
                    </div>
                  </div>
                )}
